...
parameters {
  ...
  real<lower=0.1> theta; // Negative binomial theta
...
model {
  ...
  theta ~ gamma(100, 1); // Negative binomial theta prior
  ...
  if (all_ones(N, weights) == 1) {
    home_goals ~ neg_binomial_2_log(log_lambda_home, theta);
    away_goals ~ neg_binomial_2_log(log_lambda_away, theta);
  } else {
    for (i in 1 : N) {
      target += weights[i]
                * neg_binomial_2_log_lpmf(home_goals[i] | log_lambda_home[i], theta);
      target += weights[i]
                * neg_binomial_2_log_lpmf(away_goals[i] | log_lambda_away[i], theta);
    }
  }
}
