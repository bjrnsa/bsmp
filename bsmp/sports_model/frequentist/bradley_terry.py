...
def log_likelihood_np(params, teams, data):
    ...
    for i, team in enumerate(data):
        ...
        if data[i][2] == 1:
            ...
            result = 1 - logistic
        else:  # Handle draws by considering both outcomes
            result_home_win = logistic
            result_away_win = 1 - logistic
            log_likelihood += np.log(result_home_win) + np.log(result_away_win)
            continue
...
df_team_ratings = pd.DataFrame(
    insample_team_ratings.items(), columns=["team", "rating"]
)
# Add to df
