...
class DixonColes(BaseModel):
    ...
    rho : float, default=-0.1
        Initial value for rho parameter (Dixon-Coles adjustment)
    ...
    def __init__(self, home_advantage: float = 0.25, rho: float = -0.1) -> None:
    ...
    def get_team_ratings(self) -> pd.DataFrame:
        ...
        ratings_df.loc["Home Advantage"] = [self.params_[-2], np.nan]
        ratings_df.loc["Rho"] = [self.params_[-1], np.nan]
    ...
    def _fit(self, params: np.ndarray) -> float:
        ...
        # Calculate Dixon-Coles adjustment
        dc_adj = self._rho_correction_vec(
            self.home_goals_, self.away_goals_, home_exp, away_exp, rho
        )

        # Sum log likelihood with weights
        log_likelihood = np.sum(self.weights_ * (home_llk + away_llk + np.log(dc_adj)))
    ...
    def _rho_correction_vec(
        ...
        """
        Calculate Dixon-Coles adjustment factor for football using vectorized operations.
        ...
        away_exp : np.ndarray
            Expected away goals
        rho : float
            Rho parameter for low-score adjustment

        Returns
        -------
        np.ndarray
            Adjustment factors for each match
        """
        conditions = [
            (home_goals == away_goals),
            (home_goals == (away_goals + 1)),
            (away_goals == (home_goals + 1)),
            (home_goals == (away_goals - 1)) | (away_goals == (home_goals - 1)),
        ]
